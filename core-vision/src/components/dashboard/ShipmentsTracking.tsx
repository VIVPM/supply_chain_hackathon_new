...
const statusConfig = {
  "in_transit": { color: "bg-blue-500", variant: "default" as const, icon: Truck },
  ...
  useEffect(() => {
    const fetchShipments = async () => {
      try {
        ...
        const mappedShipments = data.shipments.map(shipment => {
          const completedLegs = shipment.legs.filter(leg => leg.status === "completed").length;
          const totalLegs = shipment.legs.length;
          const progress = Math.round((completedLegs / totalLegs) * 100);

          let priority = "medium";
          if (shipment.criticality_score > 80) priority = "critical";
          else if (shipment.criticality_score > 60) priority = "high";

          let riskLevel = "low";
          if (shipment.criticality_score > 80) riskLevel = "high";
          else if (shipment.criticality_score > 50) riskLevel = "medium";

          let carrier = shipment.vendor_id;
          let vessel = shipment.ship_name || shipment.flight_number || "N/A";
          let cargo = shipment.product_id;
          let containers = shipment.container_id ? 1 : 0;

          const estimatedDelay = 0;

          return {
            id: shipment.product_id,
            origin: shipment.shipping_origin,
    ...
    <Card className="card-maritime">
      ...
      <CardContent className="space-y-3 px-4 pb-4">
        ...
        <div className="space-y-2 max-h-80 overflow-y-auto">
          {(showAll ? filteredShipments : filteredShipments.slice(0, 3)).map((shipment) => {
            const statusConf = statusConfig[shipment.status] || statusConfig["in_transit"];
        ...
        {filteredShipments.length > 3 && (
          <div className="text-center pt-2">
            ...
            >
              {showAll ? "Collapse All" : `View All (${filteredShipments.length}) Shipments`}
              {showAll ? <ChevronUp className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
            </Button>
          </div>
        )}
      </CardContent>
